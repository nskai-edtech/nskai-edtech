...
export const learningPaths = pgTable(
  ...
  (table) => [index("learning_path_published_idx").on(table.isPublished)],
);
...
export const learningPathCourses = pgTable(
  ...
  (table) => [
    index("lpc_learning_path_id_idx").on(table.learningPathId),

    uniqueIndex("lpc_path_course_unique_idx").on(
...
export const userLearningPaths = pgTable(
  ...
  (table) => [
    index("ulp_user_id_idx").on(table.userId),

    uniqueIndex("ulp_user_path_unique_idx").on(
      table.userId,
      table.learningPathId,
    ),
  ],
