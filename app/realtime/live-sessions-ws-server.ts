...
const PORT = Number(
  process.env.PORT ?? process.env.LIVE_SESSIONS_WS_PORT ?? 8080,
);
...
wss.on("connection", (socket) => {
  ...
  // Announce connection
  socket.send(JSON.stringify({ type: "PING" } satisfies LiveSessionWsEvent));
...
server.listen(PORT, () => {
  ...
  console.log(
    `[ws-server]   Broadcast: http://localhost:${PORT}/broadcast  (POST, X-Broadcast-Secret)`,
  );
