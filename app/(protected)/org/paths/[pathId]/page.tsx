...
export default async function LearningPathPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      {/* Hero Section */}
      <div className="bg-surface border-b border-border py-8 sm:py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="space-y-6">
            ...
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-black text-primary-text leading-tight">
              {path.title}
            </h1>
          ...
          <div className="relative aspect-video rounded-2xl sm:rounded-4xl overflow-hidden border-4 sm:border-8 border-surface-muted shadow-2xl">
            {path.imageUrl ? (
              <Image
                src={path.imageUrl}
                alt={path.title}
      ...
      {/* Curriculum Grid */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 mt-8 sm:mt-16">
        <div>
          <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <PathActions
                  ...
                  totalPrice={formatPrice(path.totalPrice)}
                  bundlePrice={formatPrice(path.price || path.totalPrice)}
                />
            ...
            <div className="space-y-4">
              {path.attachedCourses.map((item, index) => (
                <div
                  key={item.courseId}
                  className="group flex gap-3 sm:gap-6 p-4 sm:p-6 bg-surface border border-border rounded-2xl sm:rounded-3xl hover:border-brand/50 transition-all relative"
                >
                  <div className="shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-primary-text text-white flex items-center justify-center font-black text-lg sm:text-xl">
                    {index + 1}
                  </div>

                  <div className="flex-1 space-y-1 pr-8 sm:pr-12 md:pr-24">
                    <h3 className="text-base sm:text-xl font-bold text-primary-text group-hover:text-brand transition-colors">
                      {item.title}
                    </h3>
